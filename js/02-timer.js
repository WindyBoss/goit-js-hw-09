...
const notifyOptions = {
  ...
  onClose(selectedDates) {
    if (Date.parse(selectedDates) < currentDate) {
      Notify.failure('Please choose a date in the future');
      timer.notifyBlockAdd(refs.startBtn);
    } else if (Date.parse(selectedDates) > currentDate) {
      timer.notifyBlockRemove(refs.startBtn);
    } else if (isActive && Date.parse(selectedDates) < currentDate) {
      Notify.failure('Please choose a date in the future');
      timer.notifyBlockAdd(refs.startBtn);
    };
  },
...
const timer = new Timer({ onTick: postTimer, inputSelector: refs.futureTimeInput, btnSelector: refs.startBtn });

refs.futureTimeInput.addEventListener('change', timer.takePlannedTime.bind(timer));
